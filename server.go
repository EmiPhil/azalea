// This part is copied wholesale from the git repo for now since Azalea will need
// to completely change it once raft is working
...
func NewServer(serverId int, peers []int, storage Storage, ready <-chan interface{}, commitChan chan<- CommitEntry) *Server {
	s := new(Server)
	s.serverId = serverId
	s.peers = peers
...
func (s *Server) Serve() {
	s.mu.Lock()
	s.cm = NewConsensusModule(s.serverId, s.peers, s, s.storage, s.ready, s.commitChan)
...
func (s *Server) Call(id int, serviceMethod rpcCommand, args interface{}, reply interface{}) error {
	s.mu.Lock()
	...
	if peer == nil {
		...
		return peer.Call(string(serviceMethod), args, reply)
	}
}
...
func (rpp *RPCProxy) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) error {
	if len(os.Getenv(RaftUnreliableRPC)) > 0 {
		dice := rand.Intn(10)
		if dice == 9 {
			Log(ConsensusModuleId, Debug, "drop RequestVote")
			return fmt.Errorf("RPC failed")
		} else if dice == 8 {
			Log(ConsensusModuleId, Debug, "delay RequestVote")
...
func (rpp *RPCProxy) AppendEntries(args AppendEntriesArgs, reply *AppendEntriesReply) error {
	if len(os.Getenv(RaftUnreliableRPC)) > 0 {
		dice := rand.Intn(10)
		if dice == 9 {
			Log(ConsensusModuleId, Debug, "drop AppendEntries")
			return fmt.Errorf("RPC failed")
		} else if dice == 8 {
			Log(ConsensusModuleId, Debug, "delay AppendEntries")
			time.Sleep(75 * time.Millisecond)
		}
	} else {
		time.Sleep(time.Duration(1+rand.Intn(5)) * time.Millisecond)
